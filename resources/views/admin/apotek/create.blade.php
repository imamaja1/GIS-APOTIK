...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="rounded-2xl border border-gray-200 bg-white p-6">
        ...
        <form method="POST" action="{{ route('admin.apotek.store') }}" class="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="alamat_lengkap" rows="4" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600">{{ old('alamat_lengkap') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="input-lat" name="latitude" value="{{ old('latitude') }}" required
                            class="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Longitude</label>
                        <input type="text" id="input-lng" name="longitude" value="{{ old('longitude') }}" required
                            class="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600">
                    </div>
                </div>
            ...
            <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div
                            ...
                            <select name="jam_operasional[{{ $day }}][status_buka]"
                                ...
                                <option value="Buka" {{ old('jam_operasional.'.$day.'.status_buka')==='Buka'
                                    ? 'selected' : (old('jam_operasional')===null ? 'selected' : '' ) }}>Buka</option>
                                <option value="Tutup" {{ old('jam_operasional.'.$day.'.status_buka')==='Tutup'
                                    ? 'selected' : '' }}>Tutup</option>
                            </select>
                            <input type="time" name="jam_operasional[{{ $day }}][jam_buka]"
                                value="{{ old('jam_operasional.'.$day.'.jam_buka') }}"
                                class="h-10 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600">
                            <input type="time" name="jam_operasional[{{ $day }}][jam_tutup]"
                                value="{{ old('jam_operasional.'.$day.'.jam_tutup') }}"
                                class="h-10 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 focus:outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600">
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                    ...
                    <button type="submit"
                        class="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium cursor-pointer">Simpan</button>
                </div>
            </div>
        </form>
    </div>
